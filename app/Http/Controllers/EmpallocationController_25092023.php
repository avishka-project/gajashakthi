...
class EmpallocationController extends Controller
{
    ...
    public function insert(Request $request){
        ...
        if(!$permission) {
            ...
            foreach ($tableData as $rowtabledata) {
                ...
                $service_no = $rowtabledata['col_2'];
                $empname = $rowtabledata['col_3'];
                $subregfromtxt = $rowtabledata['col_4'];
                $subregtotxt = $rowtabledata['col_5'];
                $fromdate = $rowtabledata['col_6'];
                $todate = $rowtabledata['col_7'];
                $fromID = $rowtabledata['col_8'];
                ...
                $transferdetail->emp_subregion_from = $fromID;
                $transferdetail->from_date = $fromdate;
                $transferdetail->to_date = $todate;
    ...
    public function displaytransferlist()
    {
        ...
        ->select(
            'employeetransfers.*',
            'employeetransfers.id AS transid',
        ...
        return Datatables::of($requests)
            ...
            ->addColumn('action', function ($row) {
                ...
                $btn .= ' <button name="view" id="'.$row->id.'" class="view btn btn-dark btn-sm" type="submit"><i class="fas fa-eye"></i></button>';
    ...
    private function allocationdetailelist($id){
        ...
        ->leftjoin('employeetransfers', 'employeetransfer_details.transfer_id', '=', 'employeetransfers.id')
        ->leftjoin('subregions as subregion_from', 'employeetransfer_details.emp_subregion_from', '=', 'subregion_from.id')
        ->leftjoin('subregions as subregion_to', 'employeetransfers.subregion_id_to', '=', 'subregion_to.id')
        ->select('employeetransfer_details.*', 'subregion_from.subregion AS empsubregion','subregion_to.subregion AS empsubregionto', 'employees.emp_fullname AS empname')
        ...
        foreach ($data as $row) {
            ...
            $html .= '<td>' . $row->service_no . '</td>'; 
            $html .= '<td>' . $row->empname . '</td>'; 
            $html .= '<td>' . $row->empsubregion . '</td>'; 
            $html .= '<td>' . $row->empsubregionto. '</td>'; 
    ...
    public function deletelist(Request $request){
        ...
        $employeeTransferDetail = Employeetransfer_detail::findOrFail($id);

        // Store the record details in variables
        $empID = $employeeTransferDetail->emp_id;
        $datefrom = $employeeTransferDetail->from_date;
        $dateto = $employeeTransferDetail->to_date;



        $attendanceRecords = Empattendances::where('emp_id', $empID)
        ->whereBetween('date', [$datefrom, $dateto])
        ->get();

        if ($attendanceRecords->count() > 0) {
            
            return response()->json(['error' => 'There are attendance records for this employee during the specified period. Cannot delete.']);
        } else {
            ...
            Employeetransfer_detail::findOrFail($id)->update($form_data);
...
public function employeeselect(Request $request){
    ...
    $subregion_from = $request->input('subregion_from');
    $subregion_to = $request->input('subregion_to');
    $tosubregion = $request->input('tosubregion');
    $fromsubregion = $request->input('fromsubregion');
       
    $data = DB::table('employees')
         ->select('employees.*')
         ->addSelect(DB::raw("'$fromsubregion' as fromsubregion"))
         ->addSelect(DB::raw("'$tosubregion' as tosubregion"))
         ->where('subregion_id', '=', $subregion_from)
         ->where('deleted', '=', '0')
         ->get();
    ...
    return Datatables::of($data)
            ...
            ->addColumn('action', function ($row) {
                ...
                $btn .= ' <button name="add-row " id="'.$row->id.'" class="add-row  btn btn-primary btn-sm" type="submit"><i class="fas fa-plus"></i></button>';
...
public function getsearchempinfo(Request $request) {
    ...
    $matchingData = DB::select("
        SELECT *
        FROM employees
        WHERE subregion_id = '$areaId'
        AND (
            emp_national_id LIKE '%$searchTerm%'
            OR service_no LIKE '%$searchTerm%'
            OR emp_name_with_initial LIKE '%$searchTerm%'
        )
        LIMIT 1
    ");
