    ...
    public function upload(Request $request){
        ...
        if(!$permission) {
            ...
            if ($request->hasFile('certificates')) {
                ...
                $file->storeAs('certificates', $filename); // Store the file in storage/app/certificates directory
            }
                ...
                $filenameEmpty = empty($row->filename);
                        ...
                        $permission = $user->can('Deaddonationincomplete-edit');
                         if ($permission && !$filenameEmpty) {
                            $btn .= ' <button name="viewpdf" id="'.$row->id.'" filename="'.$row->filename.'" class="viewpdf btn btn-outline-primary btn-sm"
...
public function downloadPDF(Request $request)
    {
        ...
        // Add any necessary validation or checks here, such as verifying user access to the PDF

        $filePath = storage_path('app\certificates\\' . $filename.'.pdf');
        // dd($filePath); // Print the file path for debugging
