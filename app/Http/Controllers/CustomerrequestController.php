...
class CustomerrequestController extends Controller
{
    ...
    public function index()
    {
        ...
        if (Auth::user()->can('CustomerRequest-edit')) {
            $editpermission = 1;
        }
        if (Auth::user()->can('CustomerRequest-status')) {
            $deletepermission = 1;
        }
        if (Auth::user()->can('CustomerRequest-delete')) {
            $statuspermission = 1;
        }
        ...
        return view('Customerrequest.customerrequest', compact('customers', 'areas', 'titles', 'holidays', 'shifttypes','subcustomer','specialholidays','approvel01permission','approvel02permission','approvel03permission','listpermission','editpermission','deletepermission','statuspermission'));
    }
...
$checkcustomer=$request->input('customer');
$checkcustomerbranch=$request->input('area');
$checkfromdate=$request->input('fromdate');
$checktodate=$request->input('todate');

$overlapRecords = Customerrequest::where('customer_id', $checkcustomer)
->where('customerbranch_id', $checkcustomerbranch)
->whereIn('status', [1, 2])
    ->where(function ($query) use ($checkfromdate, $checktodate) {
        $query->where(function ($query) use ($checkfromdate, $checktodate) {
            $query->whereBetween('fromdate', [$checkfromdate, $checktodate])
                ->orWhereBetween('todate', [$checkfromdate, $checktodate]);
        })->orWhere(function ($query) use ($checkfromdate, $checktodate) {
            $query->where('fromdate', '<', $checkfromdate)
                ->where('todate', '>', $checktodate);
        });
    })
    ->exists();

if ($overlapRecords) {
    return response()->json(['status' => 2, 'message' => 'This Range allready allocated']);
} else {
                ...
                if($permission){
                    if($row->approve_01 == 0){
                        if($requestype=="Normal"){
                            ...
                            $btn .= ' <button name="special_appL1" id="'.$row->id.'" class="special_appL1 btn btn-outline-danger btn-sm" type="submit"><i class="fas fa-level-up-alt"></i></button>';
                        }
                ...
                if($permission){
                    if($row->approve_01 == 1 && $row->approve_02 == 0){
                        if($requestype=="Normal"){
                            ...
                            $btn .= ' <button name="special_appL2" id="'.$row->id.'" class="special_appL2 btn btn-outline-warning btn-sm" type="submit"><i class="fas fa-level-up-alt"></i></button>';
                        }
                    }
                }
                ...
                if($permission){
                    if($row->approve_02 == 1 && $row->approve_03 == 0 ){
                        if($requestype=="Normal"){
                            ...
                            $btn .= ' <button name="special_appL3" id="'.$row->id.'" class="special_appL3 btn btn-outline-info btn-sm" type="submit"><i class="fas fa-level-up-alt"></i></button>';  
                        }
                    }
                }
                    ...
                    if($row->approve_03 == 1 ){
                        if($requestype=="Normal"){
                           ...
                           role="button"><i class="fa fa-eye"></i></button>';
                            ...
                            $btn .= ' <button name="specialview" id="'.$row->id.'" class="specialview btn btn-outline-secondary btn-sm"
                           role="button"><i class="fa fa-eye"></i></button>';
                        }
                   }
                }
...
foreach ($titles as $title) {
 ...
 foreach ($holidays as $holiday) {
     ...
     $htmlTable .= '<td class="text-center"><input type="number" class="text-center holiday-input3" jobtitle_id3="' . $title->id . '" shift_id3="2" holiday_id2="' . $holiday->id . '" data-holiday3="' . $holiday->name . '" data-time3="day" style="width: 55px; border: none; background-color: transparent" id="' . $title->title . $holiday->name . '_day2" name="' . $title->title . $holiday->name . '_day2" value="' . $dayInputValue . '" readonly></td>';
     $htmlTable .= '<td class="text-center"><input type="number" class="text-center holiday-input3" jobtitle_id3="' . $title->id . '" shift_id3="3" holiday_id2="' . $holiday->id . '" data-holiday3="' . $holiday->name . '" data-time3="night" style="width: 55px; border: none; background-color: transparent" id="' . $title->title . $holiday->name . '_night2" name="' . $title->title . $holiday->name . '_night2" value="' . $nightInputValue . '" readonly></td>';
 }
...
public function specialapprovel_details(Request $request){
    $user = Auth::user();
    $permission =$user->can('CustomerRequest-edit');
    if(!$permission) {
            return response()->json(['error' => 'UnAuthorized'], 401);
    ...
    if (request()->ajax()){
        ...
        $requestlist = $this->specialapp_reqestcountlist($id); 
...
private function specialapp_reqestcountlist($id){
$recordID =$id ;
...
foreach ($specialholidays as $holiday) {
 ...
 $htmlTable .= '<td class="text-center"><input type="number" class="text-center speacial_holiday-input3" speacial_jobtitle_id3="' . $title->id . '" speacial_shift_id3="2" speacial_holiday_id3="' . $holiday->id . '" speacial_data-holiday3="' . $holiday->name . '" speacial_data-time3="day" style="width: 150px; border: none; background-color: transparent; margin-right:-72px;margin-left: -72px" id="' . $title->title . $holiday->name . '_day3" name="' . $title->title . $holiday->name . '_day3" value="' . $dayInputValue . '"readonly></td>';
 $htmlTable .= '<td class="text-center"><input type="number" class="text-center speacial_holiday-input3" speacial_jobtitle_id3="' . $title->id . '" speacial_shift_id3="3" speacial_holiday_id3="' . $holiday->id . '" speacial_data-holiday3="' . $holiday->name . '" speacial_data-time3="night" style="width: 150px; border: none; background-color: transparent; margin-right:-72px;margin-left: -72px" id="' . $title->title . $holiday->name . '_night3" name="' . $title->title . $holiday->name . '_night3" value="' . $nightInputValue . '"readonly></td>';
}
...
foreach ($titles as $title) {
 ...
 foreach ($specialholidays as $holiday) {
     ...
     $htmlTable .= '<td class="text-center"><input type="number" class="text-center speacial_holiday-input2" onkeyup="speacial_calculateTotal2()" speacial_jobtitle_id2="' . $title->id . '" speacial_shift_id2="2" speacial_holiday_id2="' . $holiday->id . '" speacial_data-holiday2="' . $holiday->name . '" speacial_data-time2="day" style="width: 150px; border: none; background-color: transparent; margin-right:-72px;margin-left: -72px" id="' . $title->title . $holiday->name . '_day2" name="' . $title->title . $holiday->name . '_day2" value="' . $dayInputValue . '"></td>';
     $htmlTable .= '<td class="text-center"><input type="number" class="text-center speacial_holiday-input2" onkeyup="speacial_calculateTotal2()" speacial_jobtitle_id2="' . $title->id . '" speacial_shift_id2="3" speacial_holiday_id2="' . $holiday->id . '" speacial_data-holiday2="' . $holiday->name . '" speacial_data-time2="night" style="width: 150px; border: none; background-color: transparent; margin-right:-72px;margin-left: -72px" id="' . $title->title . $holiday->name . '_night2" name="' . $title->title . $holiday->name . '_night2" value="' . $nightInputValue . '"></td>';
 }
...
public function customerrequestdocument(Request $request){
    ...
    ->leftjoin('customerbranches', 'customerrequests.customerbranch_id', '=', 'customerbranches.id')
    ->leftjoin('job_titles', 'customerrequestdetails.job_title_id', '=', 'job_titles.id')
    ->select('customerrequests.id AS requestid','customerrequests.fromdate AS fromdate','customerrequests.todate AS todate', 'customers.name AS clientname', 'customerbranches.branch_name AS branchname')
    // ->where('customerrequests.id',$id)
    ->groupBy('customerrequests.id')
    ->get(); 
    ...
    foreach ($headerdata as $datalist) {
        ...
        ->where('customerrequestdetails.customerrequest_id',$requestid)
        ->whereIn('customerrequestdetails.status', [1, 2])
        ->groupBy('customerrequestdetails.id')
        ->groupBy('customerrequestdetails.customerrequest_id')
        ->get(); 
        ...
        <table class="tg" style="table-layout: fixed; width: 100%" cellspacing="0" cellpadding="0">
                        ...
                        <tr style="text-align: center; font-size:7px; padding-top: 5px; padding-right: 30px" colspan="2">
                        <td colspan="22"><h3 class="font-weight-light">Fom:'.$fromdate.'  To:'.$todate.'</h3></td>
                    </tr> 
                        <tr style="text-align:center; font-weight:bold; font-size:5px;">
                        <th rowspan="2"style="text-align: center; font-size:10px;border: 1px solid black;border-collapse: collapse;">#</th>
                        <th rowspan="2" colspan="3"style="text-align: center; font-size:10px;border: 1px solid black; border-collapse: collapse;">Client Name</th>
                        <th rowspan="2" colspan="4"style="text-align: center; font-size:10px;border: 1px solid black; border-collapse: collapse;">Branch Name</th>
                        <th rowspan="2" colspan="2"style="text-align: center; font-size:10px;border: 1px solid black; border-collapse: collapse;">Rank </th>
                        <th colspan="2"style="text-align: center; font-size:10px;border: 1px solid black; border-collapse: collapse;">Normal Week Days</th>
                        <th colspan="2"style="text-align: center; font-size:10px;border: 1px solid black; border-collapse: collapse;">Saturdays</th>
                        <th colspan="2"style="text-align: center; font-size:10px;border: 1px solid black; border-collapse: collapse;">Sundays</th>
                        <th colspan="2"style="text-align: center; font-size:10px;border: 1px solid black; border-collapse: collapse;">Holidays</th>
                        <th colspan="2"style="text-align: center; font-size:10px;border: 1px solid black; border-collapse: collapse;">Special Days</th>
                        <th colspan="2"style="text-align: center; font-size:10px;border: 1px solid black; border-collapse: collapse;">Special Assignment Days *</th>
                    </tr>
                    <tr style="text-align:center; font-weight:bold; font-size:5px;">
                        <th style="text-align: center; font-size:10px;border: 1px solid black; border-collapse: collapse;">D</th>
                        <th style="text-align: center; font-size:10px;border: 1px solid black; border-collapse: collapse;">N</th>
                        <th style="text-align: center; font-size:10px;border: 1px solid black; border-collapse: collapse;">D</th>
                        <th style="text-align: center; font-size:10px;border: 1px solid black; border-collapse: collapse;">N</th>
                        <th style="text-align: center; font-size:10px;border: 1px solid black; border-collapse: collapse;">D</th>
                        <th style="text-align: center; font-size:10px;border: 1px solid black; border-collapse: collapse;">N</th>
    ...
    <body>
        <div id="DivIdToPrint">
            <table style="width:100%;" cellspacing="0" cellpadding="0">
                ...
                <tr>
                    <td style="text-align: center; font-size:7px; padding-top: 5px; padding-right: 10px" colspan="2">
    ...
    // return $pdf->download('AuthorizedCadre.pdf');

    // $pdf = new Dompdf();
    // $pdf->loadHTML($html);
    // $pdf->setPaper('legal', 'landscape');
    // $pdf->render();
    // $pdf->stream("AuthorizedCadre.pdf");
