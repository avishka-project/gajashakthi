...
class SalaryadvancesController extends Controller
{
    ...
    public function index()
    {
        ...
        if (in_array('Salaryadvances-edit', $userPermissions)) {
            $editpermission = 1;
        } 
        if (in_array('Salaryadvances-status', $userPermissions)) {
            $statuspermission = 1;
        } 
        if (in_array('Salaryadvances-delete', $userPermissions)) {
            $deletepermission = 1;
        } 
        ...
        return view('Salaryadvances.salaryadvance', compact('suppliers', 'employees','branches','voregions','approvel01permission','approvel02permission','approvel03permission','listpermission','editpermission','deletepermission','statuspermission','userPermissions'));
    } 
    ...
    public function insert(Request $request)
    {
            $commen= new Commen();
            $userPermissions = $commen->Allpermission();
            if (!in_array('Salaryadvances-create', $userPermissions)) {
        ...
        $accommodationfee = new Salaryadvance();
        ...
        foreach ($tableDataArray as $rowtableDataArray) {
            ...
            $amount = $rowtableDataArray['amount'];
           
            $accommodationfeesdetail = new Salaryadvancedetail();
            $accommodationfeesdetail->salaryadvance_id = $requestID;
            $accommodationfeesdetail->emp_id = $employee;
            $accommodationfeesdetail->amount = $amount;
        ...
        return response()->json(['success' => 'Salaryadvances is successfully Inserted']);
        ...
        return Datatables::of($requests)
            ...
            ->addColumn('action', function ($row) {
                ...
                if(in_array('Salaryadvances-edit',$userPermissions)){
                        ...
                        if(in_array('Salaryadvances-status',$userPermissions)){
                            if($row->status == 1){
                                $btn .= ' <a href="'.route('salaryadvancestatus', ['id' => $row->id, 'stasus' => 2]) .'" onclick="return deactive_confirm()" target="_self" class="btn btn-outline-success btn-sm mr-1 "><i class="fas fa-check"></i></a>';
                            }else{
                                $btn .= '&nbsp;<a href="'.route('salaryadvancestatus', ['id' => $row->id, 'stasus' => 1]) .'" onclick="return active_confirm()" target="_self" class="btn btn-outline-warning btn-sm mr-1 "><i class="fas fa-times"></i></a>';
                            }
                        }
    ...
    public function delete(Request $request){
        ...
        Salaryadvance::findOrFail($id)
        ->update($form_data);

        return response()->json(['success' => 'Salaryadvances is successfully Deleted']);
    ...
    public function approvel_details(Request $request){
        ...
        if (!in_array('Approve-Level-01', $userPermissions) || !in_array('Approve-Level-02', $userPermissions) || !in_array('Approve-Level-03', $userPermissions)) {
            ...
            $id = Request('id');
            if (request()->ajax()){
            $data = DB::table('salaryadvances')
            ->leftjoin('customerbranches', 'salaryadvances.location_id', '=', 'customerbranches.id')
            ->leftjoin('subregions', 'customerbranches.subregion_id', '=', 'subregions.id')
            ->select('salaryadvances.*','subregions.subregion')
            ->where('salaryadvances.id', $id)
            ->get(); 
...
private function app_reqestcountlist($id){
    ...
    $recordID =$id ;
   $data = DB::table('salaryadvancedetails')
   ->leftjoin('employees', 'salaryadvancedetails.emp_id', '=', 'employees.id')
   ->select('salaryadvancedetails.*','employees.service_no', 'employees.emp_fullname', DB::raw('(salaryadvancedetails.id) AS boardingfeesdetailsID'))
   ->where('salaryadvancedetails.salaryadvance_id', $recordID)
   ->where('salaryadvancedetails.status', 1)
   ->get(); 
   ...
   foreach ($data as $row) {
    ...
    $htmlTable .= '<td id="boardingfee">' . $row->amount . '</td>'; 
...
private function reqestcountlist($id){
    ...
    $data = DB::table('salaryadvancedetails')
    ->leftjoin('salaryadvances', 'salaryadvancedetails.salaryadvance_id', '=', 'salaryadvances.id')
    ->leftjoin('employees', 'salaryadvancedetails.emp_id', '=', 'employees.id')
    ->select('salaryadvancedetails.*','employees.service_no', 'employees.emp_fullname','salaryadvances.request_type', DB::raw('(salaryadvancedetails.id) AS boardingfeesdetailsID'))
    ->where('salaryadvancedetails.salaryadvance_id', $recordID)
    ->where('salaryadvancedetails.status', 1)
    ->get(); 
   ...
   foreach ($data as $row) {
    ...
    $htmlTable .='</td>';
    $htmlTable .= '<td>';
    $htmlTable .= '<input type="number" id="amount' . $count . '" name="amount' . $count . '" value="' . $row->amount . '" >';
    $htmlTable .= '</td>';   
    $htmlTable .= '<td class="d-none" id="empid">' . $row->emp_id . '</td>'; 
    $htmlTable .= '<td class="d-none">ExistingData</td>'; 
...
public function editlist(Request $request){
    ...
    if (request()->ajax()){
    $data = DB::table('salaryadvancedetails')
                ->select('salaryadvancedetails.*')
                ->where('salaryadvancedetails.id', $id)
                ->get(); 
...
public function update(Request $request){
    ...
    DB::table('salaryadvancedetails')
    ->where('salaryadvance_id', $hidden_id)
    ->delete();
    ...
    foreach ($tableDataArray as $rowtableDataArray) {
        ...
        $accommodationfeesdetail = new Salaryadvancedetail();
        $accommodationfeesdetail->salaryadvance_id = $hidden_id;
        $accommodationfeesdetail->emp_id = $employee;
        $accommodationfeesdetail->amount = $amount;
        $accommodationfeesdetail->status = '1';
        $accommodationfeesdetail->create_by = Auth::id();
    ...
    return response()->json(['success' => 'Salaryadvances is successfully Updated']);
...
public function approve(Request $request){
    ...
    if($applevel == 1){
        ...
        return response()->json(['success' => 'Salaryadvances is successfully Approved']);
       ...
       ->update($form_data);
        ...
        foreach ($tableDataArray as $rowtableDataArray) {
            ...
            $emp_expense->cost = $amount;
            $emp_expense->expenses_type = 'Salary_Advance';
...
public function status($id,$statusid){
    ...
    if($statusid == 1){
        ...
        return redirect()->route('salaryadvance');
    } else{
        $form_data = array(
            'status' =>  '2',
            'update_by' => Auth::id(),
...
public function deletelist(Request $request){
    ...
    Salaryadvancedetail::findOrFail($id)
    ->update($form_data);
