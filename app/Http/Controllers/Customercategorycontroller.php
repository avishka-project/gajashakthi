            ...
            ->addColumn('action', function ($row) {
                ...
                $user = Auth::user();
                    ...
                    $permission = $user->can('customercategory-status');
                        if($permission){
                            if($row->status == 1){
                                $btn .= ' <a href="'.route('cuscatstatus', ['id' => $row->id, 'stasus' => 2]) .'" onclick="return deactive_confirm()" target="_self" class="btn btn-success btn-sm mr-1 "><i class="fas fa-check"></i></a>';
                            }else{
                                $btn .= '&nbsp;<a href="'.route('cuscatstatus', ['id' => $row->id, 'stasus' => 1]) .'" onclick="return active_confirm()" target="_self" class="btn btn-warning btn-sm mr-1 "><i class="fas fa-times"></i></a>';
                            }
                        }
