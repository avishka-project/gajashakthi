...
class EmpallocationController extends Controller
{
    public function index()
    {
        ...
        if($permission){
            $customers = Subcustomer::orderBy('id', 'asc')
            ->whereIn('subcustomers.status', [1, 2])
            ->where('subcustomers.approve_status', 1)
            ->get();
     ...
     $customers = Subcustomer::orderBy('id', 'asc')
            ->leftjoin('subregions','subcustomers.subregion_id','=','subregions.id')
            ->select('subcustomers.*' )
            ->whereIn('subcustomers.status', [1, 2])
            ->where('subcustomers.approve_status', 1)
    ...
    public function addallocation($id , $action){
        ...
        $employeesspecial = DB::table('employees')
        ->leftjoin('subregions','employees.subregion_id','=','subregions.id')
       ->select('employees.*','subregions.subregion')
        ->whereIn('employees.emp_status', [1, 2])
       ->get();

        return view('EmployeeAllocation.addallocation' ,compact('id','pageaction','employees','jobtitles','employeesspecial'));
    }
    ...
    public function requestlist(Request $request){
        ...
        $requests = DB::table('customerrequests')
                    ...
                    ->select('customerrequests.*','customers.name AS namecustomer','customerbranches.branch_name AS namebranch','shift_types.shift_name AS nameshift')
            ...
            ->addColumn('action', function ($row) {
                $btn = '';
                  $btn .= '<a href="'.route('viewaallocation', ['id' => $row->id , 'action' => 1]) .'"  target="_self" class="allocate btn btn-outline-primary btn-sm"><i class="fas fa-plus"></i></a>';
            ...
            ->addColumn('action', function ($row) {
                ...
                if($permission){
                    if($row->approve_02 == 1 && $row->approve_03 == 0 ){
                        ...
                        if($permission){
                            $btn .= '&nbsp;<a href="'.route('viewaallocation', ['id' => $row->id , 'action' => 2]) .'"  target="_self" class="edit btn btn-outline-primary btn-sm"><i class="fas fa-pencil-alt"></i></a>';
                        }
    ...
    public function requestdetails(Request $request){
                ...
                $req_fromdate = $data[0]->fromdate;
                $req_todate = $data[0]->todate;

                // get the allocation date 
            $record = empallocation::where('request_id', $id)->latest()->first();
            if ($record) {
                $daterecord = $record->date;

                $carbonDate = new Carbon($daterecord);
                $dateValue = $carbonDate->addDays(1)->toDateString();

                if($req_fromdate <= $dateValue && $dateValue <= $req_todate){
                    $date_allocate =  $dateValue;
                }else{
                ...
                $date_allocate = $req_fromdate;
            }


            // get hollydate type

            $holidayType = DB::table('holidays')
            ->where('date', $date_allocate)
            ->value('holiday_type');
                ...
                if ($holidayType) {
                    ...
                    $holidayid = $holidaytypelist[0]->id;
                } else {


                    $carbonDate = new Carbon($date_allocate);
                    ...
                    $holidaytypelist2 = DB::table('holiday_types')
                    ->where('name', $holidayname)
                    ->get();
                    $holidayid = $holidaytypelist2[0]->id;

                    
                }


                            $detaillist = $this->requestdetailslist($id); 
        
                            $responseData = array(
                                'allocatedate' => $date_allocate,
                                'holidatename' => $holidayname,
                                'holidateid' => $holidayid,
                                'mainData' => $data[0],
    ...
    private function requestdetailslist($id){
        $recordID =$id ;
            ...
            $data = DB::table('customerrequests')
                        ...
                        ->where('customerrequests.id', $recordID)
                        ->get(); 
    ...
    public function insert(Request $request){
        ...
        if(!$permission) {
            ...
            $customersubregion = $request->input('subregion');
            ...
            $allocation->subregion_id = $customersubregion;
            ...
            $allocation->specialrequest_approvestatus = '0';
            $allocation->specialrequest_approve01 = '0';
            $allocation->specialrequest_approve02 = '0';
            $allocation->specialrequest_approve03 = '0';
            ...
            foreach ($tableData as $rowtabledata) {
                ...
                $empsubregion = $rowtabledata['col_5'];
    
                if($empsubregion !=  $customersubregion){
                   $specialrequest = 1;
                }else{
                    $specialrequest = 0;
                }
                ...
                $allocationdetail->specialrequest_approve_need = $specialrequest;
        ...
        if(!$permission) {
            ...
            $id = Request('id');
                $data = DB::table('empallocations')
                    ...
                    ->select('empallocations.*', 'customers.name', 'customerbranches.branch_name','subcustomers.sub_name','shift_types.shift_name','holiday_types.name AS holiday')
...
public function update(Request $request){
    ...
    $tableData = $request->input('tableData');
          ...
          $specialrequestapprove = 0;
    foreach ($tableData as $rowtabledata) {
        if(isset($rowtabledata['col_6'])){
            ...
            $originalAllocation = empallocationdetail::find($detailID);
            ...
            $allocationdetail->save();
                ...
                $originalAttributes = $originalAllocation->toArray();

                $originalAttributes['details_id'] = $detailID;
                $originalAttributes['allocation_id'] = $hidden_id;
                empallocationdetails_duplicate::create($originalAttributes);
            ...
            if($empsubregion !=  $customersubregion){
                $specialrequest = 1;
                $specialrequestapprove = 1;
             }else{
                 $specialrequest = 0;
             }

                // solution for empid passing value 0
    ...
    $allocation->approve_03 = '0';
    $allocation->status = '1';
    $allocation->specialrequest_approvestatus = $specialrequestapprove;
...
public function getempsubregion($empidId)
{
    $subregion = DB::table('employees')->select('employees.subregion_id')->where('id', '=', $empidId)->get();

    return response() ->json(['result'=> $subregion[0]]);
}
