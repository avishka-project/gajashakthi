...
class CustomerrequestController extends Controller
{
    ...
    public function insert(Request $request)
    {
        ...
        foreach ($stafflist as $rowstafflist) {

            $allocationsecuritystaff = new allocation_security_staff();
            $allocationsecuritystaff->customerrequest_id = $requestID;
            $allocationsecuritystaff->fromdate = $request->input('fromdate');
            $allocationsecuritystaff->todate = $request->input('todate');
            $allocationsecuritystaff->client_id = $request->input('customer');
            $allocationsecuritystaff->subclient_id = $request->input('subcustomer');
            $allocationsecuritystaff->branch_id = $request->input('area');
            $allocationsecuritystaff->emp_id = $rowstafflist;
            $allocationsecuritystaff->status = '1';
            $allocationsecuritystaff->create_by = Auth::id();
            $allocationsecuritystaff->update_by = '0';
            $allocationsecuritystaff->save();
        }
    ...
    $currentDate = $date->format('Y-m-d');
        ...
        foreach ($tableData as $rowtabledata) {
            ...
            $holidaytypelist2 = DB::table('holiday_types')
                    ->where('id', $holiday)
                    ->get();
                    $holidaytypename = $holidaytypelist2[0]->name;
                    $holidaytypeid =  $holiday ;

            if( $holidaytypename=="Weekend"){
                $carbonDate = Carbon::createFromFormat('Y-m-d', $currentDate);

    if ($carbonDate->isWeekend()) {
        // $currentDate is a weekend

        $allocation = new empallocation();
            $allocation->request_id =$requestID ;
            $allocation->customer_id = $request->input('customer');
            $allocation->subcustomer_id = $request->input('subcustomer');
            $allocation->customerbranch_id = $request->input('area');
            $allocation->subregion_id = $request->input('subregion_id');
            $allocation->date = $currentDate;
            $allocation->holiday_id = $holiday;
            $allocation->shift_id =  $shift;
            $allocation->jobtitle_id = $title;
            ...
            if( $holidaytypename=="Weekday"){
                $date = Carbon::createFromFormat('Y-m-d', $currentDate);

    if ($date->isWeekday()) {
        // $currentDate is a weekday
        $allocation = new empallocation();
            $allocation->request_id =$requestID ;
            $allocation->customer_id = $request->input('customer');
            $allocation->subcustomer_id = $request->input('subcustomer');
            ...
            if($holidaytypename=="Poya Holiday"){
               
                $holidaylist = DB::table('holidays')
                    ->where('holiday_type',  $holidaytypeid)
                    ->get();

                    foreach ($holidaylist as $getholiday) {
                        $holidaydate = $getholiday->date;
 
                        if ($currentDate == $holidaydate) {
                           // $currentDate is a holiday
        $allocation = new empallocation();
        $allocation->request_id =$requestID ;
        $allocation->customer_id = $request->input('customer');
        $allocation->subcustomer_id = $request->input('subcustomer');
        ...
        $allocation->save();
            ...
            if($holidaytypename=="Public & Bank Holiday"){
        ...
        $allocation->save();
            ...
            if($holidaytypename=="Public, Bank, Mercantile Holiday"){
   ...
   public function editlist(Request $request){
    ...
    public function update(Request $request){
        ...
        foreach ($stafflist as $rowstafflist) {
            ...
            $allocationsecuritystaff = new allocation_security_staff();
            $allocationsecuritystaff->customerrequest_id = $hidden_id;
            $allocationsecuritystaff->fromdate = $request->input('fromdate');
            $allocationsecuritystaff->todate = $request->input('todate');
            $allocationsecuritystaff->client_id = $request->input('customer');
            $allocationsecuritystaff->subclient_id = $request->input('subcustomer');
    ...
    allocation_security_staff::where('customerrequest_id', $hidden_id)
    ->update([
        'customerrequest_id' => $hidden_id,
        'fromdate' => $request->input('fromdate'),
        'todate' => $request->input('todate'),
        'client_id' => $request->input('customer'),
        'subclient_id' => $request->input('subcustomer'),
        'branch_id' => $request->input('area'),
        'update_by' => Auth::id(),
    ]);

    DB::table('empallocations')
    ->where('empallocations.request_id', $hidden_id)
    ->delete();
...
if ($carbonDate->isWeekend()) {
    ...
    $allocation = new empallocation();
        $allocation->request_id =$hidden_id ;
        $allocation->customer_id = $request->input('customer');
        $allocation->subcustomer_id = $request->input('subcustomer');
        $allocation->customerbranch_id = $request->input('area');
        $allocation->subregion_id = $request->input('subregion_id');
