            ...
            ->addColumn('action', function ($row) {
                ...
                if($permission){
                    if($row->approve_02 == 1 && $row->approve_03 == 0 ){
                        ...
                        $btn .= ' <a href="'.route('subcuscontact',['id' => $row->id]) .'" target="_self" class="btn btn-info btn-sm mr-1 "><i class="fas fa-phone"></i></a>';
                    
                        $permission = $user->can('subcustomer-status');
                        if($permission){
                            if($row->status == 1){
                                $btn .= ' <a href="'.route('subcustomerstatus', ['id' => $row->id, 'stasus' => 2]) .'" onclick="return deactive_confirm()" target="_self" class="btn btn-success btn-sm mr-1 "><i class="fas fa-check"></i></a>';
                            }else{
                                $btn .= '&nbsp;<a href="'.route('subcustomerstatus', ['id' => $row->id, 'stasus' => 1]) .'" onclick="return active_confirm()" target="_self" class="btn btn-warning btn-sm mr-1 "><i class="fas fa-times"></i></a>';
                            }
                        }
