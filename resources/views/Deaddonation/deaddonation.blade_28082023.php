                                ...
                                <div class="form-row mb-1">
                                    <div class="col-4">
                                        ...
                                        <select name="serviceno" id="serviceno" class="form-control form-control-sm"
                                            required onchange="getEmpName()">
                                            <option value="">Select Service No</option>
                                            @foreach($employees as $employee)
                                            <option value="{{$employee->id}}">{{$employee->emp_id}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-8">
...
<script>
        ...
        $(document).on('click', '.edit', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#serviceno').val(data.result.employee_id);
    ...
    function getEmpNameforApprove(empid) {
        ...
        $.ajax({
            ...
            success: function (data) {
                $('#app_empname').val(data.result.emp_name_with_initial);
                $('#app_serviceno').val(data.result.emp_id);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function (index, relative) {
                    var optionText = relative.emp_dep_name + '  -  ' + relative.emp_dep_relation;
                    var optionValue = relative.id;
                    var optionHtml = '<option value="' + optionValue + '">' + optionText + '</option>';

                    if (relative.life_status === "dead") {
                        optionHtml = '<option value="' + optionValue + '" disabled style="color: red;">' + optionText + ' (Dead)</option>';
                    }
    ...
    if (empId !== '') {
        $.ajax({
            ...
            success: function (data) {
                $('#relatives').empty().append('<option value="">Select Relatives</option>');
                $.each(data, function (index, relative) {
                    $('#relatives').append('<option value="' + relative.id + '">' + relative.emp_dep_name + ' - ' + relative.emp_dep_relation + '</option>');
                });
    ...
    if (empId !== '') {
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function (index, relative) {
                    $('#app_relatives').append('<option value="' + relative.id + '">' + relative.emp_dep_name + ' - ' + relative.emp_dep_relation + '</option>');
                });
